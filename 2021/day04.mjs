...
const input = (await getInput(4)).split('\n\n')

const chosen = input[0].split(',').map(Number)
const boards = input.slice(1).map(board =>
   board
      ...
      .split(' ')
      .filter(x => x)
      .map(n => ({ val: Number(n), marked: false }))
)

const isWinner = (board, i) => {
   let vert = true
   for (let j = i - (i % 5); j < i + (5 - (i % 5)); j++)
      if (!board[j].marked) vert = false
   let horiz = true
   for (let j = i % 5; j < 25; j += 5) if (!board[j].marked) horiz = false
   return vert || horiz
}
